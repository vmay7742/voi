...
sudo xbps-install -u xbps && sudo xbps-install -Su && sudo xbps-install -Suv
...
sudo xbps-install -Su xorg libX11-devel libXinerama-devel libXft-devel base-devel bspwm picom sxhkd polybar dmenu rofi feh tmux p7zip curl NetworkManager htop inxi lm_sensors pulseaudio pulsemixer wget bc fzf xclip plocate bash-completion noto-fonts-ttf noto-fonts-cjk dejavu-fonts-ttf noto-fonts-ttf-variable scrot Thunar thunar-volman gvfs gvfs-mtp elogind mtpfs ntfs-3g udisks2 gnome-themes-extra papirus-icon-theme ffmpeg ffmpegthumbnailer tumbler mousepad xdotool brillo kitty firefox
sudo xbps-install -Su mediainfo yt-dlp qemu ufw sct rxvt-unicode mpv nsxiv nomacs blender darktable hugin gimp gmic-gimp krita inkscape ImageMagick dcraw xarchiver libreoffice zathura zathura-pdf-mupdf zathura-djvu zathura-ps zathura-cb obs qbittorrent syncthing keepassxc audacity android-tools android-udev-rules kdeconnect dunst ntp openvpn acpi scrcpy meson ninja libavcodec libavdevice libavformat libavutil libswresample wine wine-mono wine-gecko winetricks
